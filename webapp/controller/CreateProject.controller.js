sap.ui.define([
	...
	"sap/m/MessageToast",
		"ehb/Project/model/Service",	"sap/ui/model/json/JSONModel",
			"ehb/Project/model/formatter", "sap/m/MessageBox"
], function (BaseController, MessageToast,Service,JSONModel,formatter,MessageBox) {
	"use strict";

	return BaseController.extend("ehb.Project.controller.CreateProject", {
		...
		maakProj: function(){
			...
			var naam = this.getView().byId("naam").getValue();
			var begindatum = this.getView().byId("begindatum").getDateValue();
		 var einddatum	= this.getView().byId("einddatum").getDateValue();
		...
		var properties = {
			...
			this.getView().getModel().create("/projectSet",properties,
			{
		...
		,		onInit: function () {
			var that = this;
			this.getRouter().getTargets().getTarget("createProject");
